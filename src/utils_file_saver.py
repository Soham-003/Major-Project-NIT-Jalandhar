...
def set_file_metadata(filepath, metadata):
    ...
    # os.setxattr is not available on Windows; only set xattrs when supported
    if hasattr(os, "setxattr"):
        try:
            os.setxattr(str(filepath), METADATA_FIELD_TAGS, metadata_bytes)
        except Exception:
            # ignore failures to keep saving robust across platforms
            pass
